...
export async function POST(request: NextRequest) {
  ...
  const cacheKey = `ads_trends:${keywords.map(encodeURIComponent).sort().join('|')}`
  ...
  try {
    ...
    const adsData = topAds.map((ad, i) => ({
      index: i + 1,
      title: sanitiseAdText(ad.ad_creative_link_titles?.[0], 300),
      body: sanitiseAdText(ad.ad_creative_bodies?.[0], 500),
    }))
